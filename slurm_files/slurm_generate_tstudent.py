...
def process_system_tstudent(args):
    """
    Process a single Student's t-distribution system.
    ...
    df = np.random.uniform(df_range[0], df_range[1])

    # Generate X from t-distribution and standardize
    # Mean is 0 for df > 1, Variance is df/(df-2) for df > 2
    X_raw = np.random.standard_t(df, size=(n, p))
...
def generate_tstudent_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], 
                                           df_range=(3, 10), show_progress=True, 
    ...
    """
    Generates data with t-student predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        folder_prefix = f"(tstudent_multilevel_{multilevel})_" if multilevel > 1 else "(tstudent)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_tstudent, system_args), total=len(local_systems), 
            ...
            results = [process_system_tstudent(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Student's t-distribution data for FDP prediction using hybrid MPI+multiprocessing")
    ...
    parser.add_argument('--df_min', type=float, default=3.0, help="Minimum degrees of freedom for Student's t-distribution")
    parser.add_argument('--df_max', type=float, default=10.0, help="Maximum degrees of freedom for Student's t-distribution")
    ...
    if comm.Get_rank() == 0:
        if args.df_min <= 2.0:
            parser.error("df_min must be greater than 2.0 for finite variance.")
    
    generate_tstudent_FDP_labeling_hybrid(
