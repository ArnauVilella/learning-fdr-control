...
def process_system_gamma(args):
    """
    Process a single gamma system.
    ...
    # Sample shape for the gamma distribution
    shape = np.random.uniform(shape_range[0], shape_range[1])
    scale = 1.0
    if shape <= 0:
        raise ValueError("Shape must be positive.")

    # Generate X from gamma distribution, then standardize
    X_raw = np.random.gamma(shape, scale=scale, size=(n, p))
...
def generate_gamma_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], shape_range=(0.5, 10.0), show_progress=True, 
    ...
    """
    Generates data with gamma predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        param_order = [
            ...
            f"p={p}", f"K={K}", f"shape_min={shape_range[0]}", f"shape_max={shape_range[1]}",
        ]
        folder_prefix = f"(gamma_multilevel_{multilevel})_" if multilevel > 1 else "(gamma)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_gamma, system_args), total=len(local_systems), 
            ...
            results = [process_system_gamma(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate gamma data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--shape_min', type=float, default=0.5, help='Minimum shape for gamma distribution')
    parser.add_argument('--shape_max', type=float, default=10.0, help='Maximum shape for gamma distribution')
    ...
    generate_gamma_FDP_labeling_hybrid(
