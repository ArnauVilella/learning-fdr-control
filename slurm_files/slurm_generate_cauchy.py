...
def process_system_cauchy(args):
    """
    Process a single cauchy system.
    ...
    clip_neg_range = args['clip_neg_range']
    clip_pos_range = args['clip_pos_range']
    ...
    # Sample asymmetric clipping thresholds
    neg_M = np.random.uniform(clip_neg_range[0], clip_neg_range[1])
    pos_M = np.random.uniform(clip_pos_range[0], clip_pos_range[1])
    if neg_M <= 0 or pos_M <= 0:
        raise ValueError("Clipping thresholds must be positive.")

    # Generate X from standard Cauchy, clip, and then standardize
    X_raw = np.random.standard_cauchy(size=(n, p))
    X_clipped = np.clip(X_raw, -neg_M, pos_M)
    
    # Standardize the clipped data to have mean 0 and variance 1
    mean_clipped = np.mean(X_clipped)
    std_clipped = np.std(X_clipped)
    if std_clipped > 1e-6:
        X = (X_clipped - mean_clipped) / std_clipped
    else:
        X = X_clipped - mean_clipped # Avoid division by zero if std is tiny
...
def generate_cauchy_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], 
                                           clip_neg_range=(10.0, 50.0), clip_pos_range=(10.0, 50.0),
    ...
    """
    Generates data with cauchy predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        param_order = [
            ...
            f"p={p}", f"K={K}", 
            f"clip_neg_min={clip_neg_range[0]}", f"clip_neg_max={clip_neg_range[1]}",
            f"clip_pos_min={clip_pos_range[0]}", f"clip_pos_max={clip_pos_range[1]}",
        ]
        folder_prefix = f"(cauchy_multilevel_{multilevel})_" if multilevel > 1 else "(cauchy)_"
        if test:
    ...
    if local_systems:
        ...
        system_args = [{'system_id': sid, 'n': n, 'p': p, 'K': K, 'T_stop_max': T_stop_max,
                        ...
                        'clip_neg_range': clip_neg_range, 'clip_pos_range': clip_pos_range,
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_cauchy, system_args), total=len(local_systems), 
            ...
            results = [process_system_cauchy(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate cauchy data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--clip_neg_min', type=float, default=10.0, help='Minimum for the negative clipping threshold range')
    parser.add_argument('--clip_neg_max', type=float, default=50.0, help='Maximum for the negative clipping threshold range')
    parser.add_argument('--clip_pos_min', type=float, default=10.0, help='Minimum for the positive clipping threshold range')
    parser.add_argument('--clip_pos_max', type=float, default=50.0, help='Maximum for the positive clipping threshold range')
    ...
    if args.clip_neg_min <= 0 or args.clip_pos_min <= 0:
        comm = MPI.COMM_WORLD
        if comm.Get_rank() == 0:
            parser.error("Clipping range minimums must be greater than 0.")
    
    generate_cauchy_FDP_labeling_hybrid(
        ...
        num_dummies_factor=args.num_dummies_factor,
        SNR_range=args.SNR_range,
        clip_neg_range=(args.clip_neg_min, args.clip_neg_max),
        clip_pos_range=(args.clip_pos_min, args.clip_pos_max),
