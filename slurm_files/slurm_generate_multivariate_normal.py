...
def process_system_multivariate_normal(args):
    """
    Process a single multivariate normal system.
    ...
    sparsity_range = args['sparsity_range']
    ...
    # Sample correlation parameter sparsity
    sparsity = np.random.uniform(sparsity_range[0], sparsity_range[1])

    # Generate X from multivariate normal distribution
    mean = np.zeros(p)
    A = np.random.uniform(-0.5, 0.5, size=(p, p))
    A = (A + A.T) / 2  # symmetrize
    np.fill_diagonal(A, 1.0)
    
    # Step 2: Impose sparsity
    mask = np.random.rand(p, p) < sparsity
    A[mask] = 0
    A = (A + A.T) / 2
    np.fill_diagonal(A, 1.0)

    # Step 3: Ensure positive definite via shrinkage
    # (Graphical Lasso forces positive-definite sparse covariance)
    model = GraphicalLasso(alpha=0.1, max_iter=200)
    X_init = np.random.randn(500, p)
    model.fit(X_init @ A)  
...
def generate_multivariate_normal_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], 
                                           sparsity_range=(0.1, 0.9), show_progress=True, 
    ...
    """
    Generates data with multivariate normal predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        param_order = [
            ...
            f"p={p}", f"K={K}", f"sparsity_min={sparsity_range[0]}", f"sparsity_max={sparsity_range[1]}"
        ]
        folder_prefix = f"(multivariate_normal_multilevel_{multilevel})_" if multilevel > 1 else "(multivariate_normal)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_multivariate_normal, system_args), total=len(local_systems), 
            ...
            results = [process_system_multivariate_normal(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate Multivariate Normal data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--sparsity_min', type=float, default=0.1, help='Minimum sparsity for multivariate normal distribution')
    parser.add_argument('--sparsity_max', type=float, default=0.9, help='Maximum sparsity for multivariate normal distribution')
    ...
    comm = MPI.COMM_WORLD
    if comm.Get_rank() == 0:
        lower_bound = -1 / (args.p - 1)
        if not (lower_bound <= args.sparsity_min <= 1 and lower_bound <= args.sparsity_max <= 1):
            parser.error(f"sparsity values must be between {lower_bound:.3f} and 1 for p={args.p}")
    
    generate_multivariate_normal_FDP_labeling_hybrid(
        ...
        num_dummies_factor=args.num_dummies_factor,
        SNR_range=args.SNR_range,
        sparsity_range=(args.sparsity_min, args.sparsity_max),
