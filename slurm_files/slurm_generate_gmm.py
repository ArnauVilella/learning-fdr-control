...
def process_system_gmm(args):
    """
    Process a single Gaussian Mixture Model (GMM) system.
    ...
    # 1. Sample GMM parameters
    k = np.random.randint(k_range[0], k_range[1] + 1)
    weights = np.random.dirichlet(np.ones(k))
    means = np.random.uniform(mean_range[0], mean_range[1], size=k)
    stds = np.random.uniform(std_range[0], std_range[1], size=k)

    # 2. Generate data from the GMM
    component_choices = np.random.choice(k, size=(n, p), p=weights)
    X_raw = np.random.normal(loc=means[component_choices], scale=stds[component_choices])

    # 3. Standardize the GMM data
    gmm_mean = np.sum(weights * means)
    gmm_var = np.sum(weights * (means**2 + stds**2)) - gmm_mean**2
    gmm_std = np.sqrt(gmm_var)
    
    if gmm_std > 1e-6:
        X = (X_raw - gmm_mean) / gmm_std
    else:
        X = X_raw - gmm_mean
...
def generate_gmm_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                     N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                     num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], 
                                     k_range=(2, 5), mean_range=(-10.0, 10.0), std_range=(0.5, 2.0),
    ...
    """
    Generates data with GMM predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        param_order = [
            ...
            f"p={p}", f"K={K}", 
            f"k_min={k_range[0]}", f"k_max={k_range[1]}",
            f"mean_min={mean_range[0]}", f"mean_max={mean_range[1]}",
            f"std_min={std_range[0]}", f"std_max={std_range[1]}",
        ]
        folder_prefix = f"(gmm_multilevel_{multilevel})_" if multilevel > 1 else "(gmm)_"
        if test:
    ...
    if local_systems:
        ...
        system_args = [{'system_id': sid, 'n': n, 'p': p, 'K': K, 'T_stop_max': T_stop_max,
                        ...
                        'k_range': k_range, 'mean_range': mean_range, 'std_range': std_range,
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_gmm, system_args), total=len(local_systems), 
            ...
            results = [process_system_gmm(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate GMM data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--k_min', type=int, default=2, help='Minimum number of GMM components')
    parser.add_argument('--k_max', type=int, default=5, help='Maximum number of GMM components')
    parser.add_argument('--mean_min', type=float, default=-10.0, help='Minimum for GMM component mean range')
    parser.add_argument('--mean_max', type=float, default=10.0, help='Maximum for GMM component mean range')
    parser.add_argument('--std_min', type=float, default=0.5, help='Minimum for GMM component std dev range')
    parser.add_argument('--std_max', type=float, default=2.0, help='Maximum for GMM component std dev range')
    ...
    if args.k_min < 1 or args.std_min <= 0:
        comm = MPI.COMM_WORLD
        if comm.Get_rank() == 0:
            parser.error("k_min must be >= 1 and std_min must be > 0.")
    
    generate_gmm_FDP_labeling_hybrid(
        ...
        mean_range=(args.mean_min, args.mean_max),
        std_range=(args.std_min, args.std_max),
