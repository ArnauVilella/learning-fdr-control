...
def process_system_lognormal(args):
    """
    Process a single lognormal system.
    ...
    # Generate X from lognormal distribution and standardize
    X_raw = np.random.lognormal(mean=mu, sigma=sigma, size=(n, p))
...
def generate_lognormal_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], 
                                           sigma_range=(0.25, 2.0), show_progress=True, 
    ...
    """
    Generates data with lognormal predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        param_order = [
            ...
            f"p={p}", f"K={K}", f"sigma_min={sigma_range[0]}", f"sigma_max={sigma_range[1]}"
        ]
        folder_prefix = f"(lognormal_multilevel_{multilevel})_" if multilevel > 1 else "(lognormal)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_lognormal, system_args), total=len(local_systems), 
            ...
            results = [process_system_lognormal(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate Lognormal data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--sigma_min', type=float, default=0.25, help='Minimum sigma for lognormal distribution')
    parser.add_argument('--sigma_max', type=float, default=2.0, help='Maximum sigma for lognormal distribution')
    ...
    if comm.Get_rank() == 0:
        if args.sigma_min <= 0:
            parser.error("sigma_min must be greater than 0.")
    
    generate_lognormal_FDP_labeling_hybrid(
