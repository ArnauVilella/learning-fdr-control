...
def process_system_gaussian(args):
    """
    Process a single Gaussian system.
...
def generate_gaussian_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
    ...
    """
    Generates beta, Phi, v, T_stop, FDP entries dataset using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        folder_prefix = f"(gaussian_multilevel_{multilevel})_" if multilevel > 1 else "(gaussian)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_gaussian, system_args), total=len(local_systems), desc=pbar_desc, disable=not (show_progress and rank == 0))
                results = list(pbar)
        else:
            pbar_desc = f"MPI Process {rank} (Single-threaded)"
            pbar = tqdm(system_args, desc=pbar_desc, disable=not (show_progress and rank == 0))
            results = [process_system_gaussian(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate Gaussian data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    data_dir = generate_gaussian_FDP_labeling_hybrid(
