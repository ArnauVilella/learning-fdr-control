...
def process_system_weibull(args):
    """
    Process a single Weibull system.
    ...
    # Sample Weibull shape parameter a
    a = np.random.uniform(a_range[0], a_range[1])

    # Generate X from Weibull distribution and standardize
    X_raw = np.random.weibull(a, size=(n, p))
    
    mean = gamma(1 + 1/a)
    variance = gamma(1 + 2/a) - (gamma(1 + 1/a))**2
...
def generate_weibull_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], 
                                           a_range=(0.5, 5.0), show_progress=True, 
    ...
    """
    Generates data with Weibull predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        folder_prefix = f"(weibull_multilevel_{multilevel})_" if multilevel > 1 else "(weibull)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_weibull, system_args), total=len(local_systems), 
            ...
            results = [process_system_weibull(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate Weibull data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--a_min', type=float, default=0.5, help='Minimum shape parameter a for Weibull distribution')
    parser.add_argument('--a_max', type=float, default=5.0, help='Maximum shape parameter a for Weibull distribution')
    ...
    if comm.Get_rank() == 0:
        if args.a_min <= 0:
            parser.error("a_min must be greater than 0.")
    
    generate_weibull_FDP_labeling_hybrid(
