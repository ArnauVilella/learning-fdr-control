...
def process_system_binomial(args):
    """
    Process a single binomial system.
    ...
    n_trials_range = args['n_trials_range']
    p_prob_range = args['p_prob_range']
    ...
    # Sample binomial parameters
    n_trials = np.random.randint(n_trials_range[0], n_trials_range[1] + 1)
    p_prob = np.random.uniform(p_prob_range[0], p_prob_range[1])

    # Generate X from binomial distribution and standardize
    X_raw = np.random.binomial(n_trials, p_prob, size=(n, p))
    
    mean = n_trials * p_prob
    variance = n_trials * p_prob * (1 - p_prob)
...
def generate_binomial_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], 
                                           n_trials_range=(10, 100), p_prob_range=(0.1, 0.9), show_progress=True, 
    ...
    """
    Generates data with binomial predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        param_order = [
            ...
            f"p={p}", f"K={K}", f"n_trials_min={n_trials_range[0]}", f"n_trials_max={n_trials_range[1]}",
            f"p_prob_min={p_prob_range[0]:.1f}", f"p_prob_max={p_prob_range[1]:.1f}"
        ]
        folder_prefix = f"(binomial_multilevel_{multilevel})_" if multilevel > 1 else "(binomial)_"
        if test:
    ...
    if local_systems:
        ...
        system_args = [{'system_id': sid, 'n': n, 'p': p, 'K': K, 'T_stop_max': T_stop_max,
                        ...
                        'n_trials_range': n_trials_range, 'p_prob_range': p_prob_range,
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_binomial, system_args), total=len(local_systems), 
            ...
            results = [process_system_binomial(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate Binomial data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--n_trials_min', type=int, default=10, help='Minimum number of trials for binomial distribution')
    parser.add_argument('--n_trials_max', type=int, default=100, help='Maximum number of trials for binomial distribution')
    parser.add_argument('--p_prob_min', type=float, default=0.1, help='Minimum probability for binomial distribution')
    parser.add_argument('--p_prob_max', type=float, default=0.9, help='Maximum probability for binomial distribution')
    ...
    comm = MPI.COMM_WORLD
    if comm.Get_rank() == 0:
        if args.n_trials_min <= 0:
            parser.error("n_trials_min must be greater than 0.")
        if not (0 <= args.p_prob_min <= 1 and 0 <= args.p_prob_max <= 1):
            parser.error("p_prob values must be between 0 and 1.")
    
    generate_binomial_FDP_labeling_hybrid(
        ...
        num_dummies_factor=args.num_dummies_factor,
        SNR_range=args.SNR_range,
        n_trials_range=(args.n_trials_min, args.n_trials_max),
        p_prob_range=(args.p_prob_min, args.p_prob_max),
