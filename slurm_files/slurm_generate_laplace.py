...
def process_system_laplace(args):
    """
    Process a single Laplace system.
    ...
    # Generate X from Laplace distribution and standardize
    # A Laplace distribution with loc=0 and scale=b has variance 2*b^2.
    # To get variance 1, we need b = 1/sqrt(2).
    # We generate with loc=0, scale=1/sqrt(2) to have mean 0 and variance 1.
    X = np.random.laplace(loc=0., scale=1./np.sqrt(2), size=(n, p))
...
def generate_laplace_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
    ...
    """
    Generates data with Laplace predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        folder_prefix = f"(laplace_multilevel_{multilevel})_" if multilevel > 1 else "(laplace)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_laplace, system_args), total=len(local_systems), desc=pbar_desc, disable=not (show_progress and rank == 0))
                results = list(pbar)
        else:
            pbar_desc = f"MPI Process {rank} (Single-threaded)"
            pbar = tqdm(system_args, desc=pbar_desc, disable=not (show_progress and rank == 0))
            results = [process_system_laplace(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate Laplace data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    data_dir = generate_laplace_FDP_labeling_hybrid(
