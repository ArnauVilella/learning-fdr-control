...
def process_system_multivariate_normal_block_discrete(args):
    """
    Process a single multivariate normal system with block correlation and discrete conversion.
    ...
    # Generate X from multivariate normal distribution with block correlation
    block_size = np.random.randint(p//250, p//50)
    block_corr = np.random.uniform(0.2, 0.75)

    corr_block = np.full((block_size, block_size), block_corr)
    np.fill_diagonal(corr_block, 1)

    # Tile this block to create a block-diagonal correlation matrix for all SNPs
    n_blocks = p // block_size
    R = block_diag(*([corr_block] * n_blocks))
    # Add remaining SNPs with no correlation if n_snps is not divisible by block_size
    remaining_snps = p % block_size
    if remaining_snps > 0:
        R_rem = np.eye(remaining_snps)
        R = block_diag(R, R_rem)

    # 2. Generate Continuous Data from MVN
    mean = np.zeros(p)
    # This generates the continuous, correlated data
    X_continuous = np.random.multivariate_normal(mean, R, size=n)

    X_percentiles = norm.cdf(X_continuous)

    mafs = np.random.uniform(0.05, 0.5, size=p)

    p2_thresholds = mafs**2
    p1_thresholds = p2_thresholds + 2 * mafs * (1 - mafs)

    X_discrete = np.zeros_like(X_percentiles, dtype=np.int8)

    # Apply thresholds to convert percentiles to 0, 1, 2
    X_discrete[X_percentiles > p1_thresholds] = 2
    X_discrete[(X_percentiles > p2_thresholds) & (X_percentiles <= p1_thresholds)] = 1
    
    scaler = StandardScaler()
    X = scaler.fit_transform(X_discrete)
...
def generate_multivariate_normal_block_discrete_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
    ...
    """
    Generates data with multivariate normal predictors with block correlation and discrete conversion using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        folder_prefix = f"(multivariate_normal_block_discrete_multilevel_{multilevel})_" if multilevel > 1 else "(multivariate_normal_block_discrete)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_multivariate_normal_block_discrete, system_args), total=len(local_systems), 
            ...
            results = [process_system_multivariate_normal_block_discrete(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate Multivariate Normal data with block correlation and discrete conversion for FDP prediction using hybrid MPI+multiprocessing')
    ...
    generate_multivariate_normal_block_discrete_FDP_labeling_hybrid(
