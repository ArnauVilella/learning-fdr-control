...
def process_system_multivariate_normal_blocks(args):
    """
    Process a single multivariate normal system with block correlation.
    ...
    block_size = np.random.randint(p//100, p//20)
    block_corr = np.random.uniform(0.4, 0.85)
    ...
    X_raw = np.random.multivariate_normal(mean, R, size=n)
...
def generate_multivariate_normal_blocks_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
    ...
    """
    Generates data with multivariate normal predictors with block correlation using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        folder_prefix = f"(multivariate_normal_blocks_multilevel_{multilevel})_" if multilevel > 1 else "(multivariate_normal_blocks)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_multivariate_normal_blocks, system_args), total=len(local_systems), 
            ...
            results = [process_system_multivariate_normal_blocks(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate Multivariate Normal data with block correlation for FDP prediction using hybrid MPI+multiprocessing')
    ...
    generate_multivariate_normal_blocks_FDP_labeling_hybrid(
