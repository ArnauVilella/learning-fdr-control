...
def process_system_gumbel(args):
    """
    Process a single gumbel system.
    ...
    # Generate X from gumbel distribution and standardize
    # For standard Gumbel(0,1), mean is euler_gamma, variance is pi^2/6
    loc = 0.0
    scale = 1.0
    X_raw = np.random.gumbel(loc=loc, scale=scale, size=(n, p))
    mean = np.euler_gamma
    std_dev = np.pi / np.sqrt(6)
    X = (X_raw - mean) / std_dev # Standardize to mean=0, variance=1
...
def generate_gumbel_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
    ...
    """
    Generates data with gumbel predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        folder_prefix = f"(gumbel_multilevel_{multilevel})_" if multilevel > 1 else "(gumbel)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_gumbel, system_args), total=len(local_systems), 
            ...
            results = [process_system_gumbel(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate gumbel data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    data_dir = generate_gumbel_FDP_labeling_hybrid(
