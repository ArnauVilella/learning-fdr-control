...
def process_system_chisquare(args):
    """
    Process a single chi-square system.
    ...
    # Generate X from chi-square distribution and y with Gaussian noise
    X_raw = np.random.chisquare(df, size=(n, p))
    X = (X_raw - df) / np.sqrt(2 * df) # Standardize to mean=0, variance=1
...
def generate_chisquare_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], df_range=(5, 10), show_progress=True, 
    ...
    """
    Generates data with chi-square predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        param_order = [
            ...
            f"p={p}", f"K={K}", f"df_min={df_range[0]}", f"df_max={df_range[1]}",
        ]
        folder_prefix = f"(chisquare_multilevel_{multilevel})_" if multilevel > 1 else "(chisquare)_"
        if test:
    ...
    if local_systems:
        ...
        system_args = [{'system_id': sid, 'n': n, 'p': p, 'K': K, 'T_stop_max': T_stop_max,
                        'num_act_range': num_act_range, 'num_dummies': num_dummies, 'num_dummies_factor': num_dummies_factor, 'SNR_range': SNR_range, 'df_range': df_range,
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_chisquare, system_args), total=len(local_systems), 
            ...
            results = [process_system_chisquare(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate chi-square data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--df_min', type=int, default=5, help='Minimum degrees of freedom for chi-square distribution')
    parser.add_argument('--df_max', type=int, default=10, help='Maximum degrees of freedom for chi-square distribution')
    ...
    if args.df_min <= 0:
        comm = MPI.COMM_WORLD
        if comm.Get_rank() == 0:
            parser.error("df_min must be greater than 0.")
    
    data_dir = generate_chisquare_FDP_labeling_hybrid(
        ...
        num_dummies_factor=args.num_dummies_factor,
        SNR_range=args.SNR_range,
        df_range=(args.df_min, args.df_max),
