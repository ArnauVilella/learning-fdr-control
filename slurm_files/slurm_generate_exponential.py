...
def process_system_exponential(args):
    """
    Process a single exponential system.
    ...
    # Generate X from exponential distribution and standardize
    # Mean is scale (1.0), variance is scale^2 (1.0)
    scale = 1.0
    X_raw = np.random.exponential(scale=scale, size=(n, p))
    X = (X_raw - scale) / scale # Standardize to mean=0, variance=1
...
def generate_exponential_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
    ...
    """
    Generates data with exponential predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        folder_prefix = f"(exponential_multilevel_{multilevel})_" if multilevel > 1 else "(exponential)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_exponential, system_args), total=len(local_systems), 
            ...
            results = [process_system_exponential(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate exponential data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    generate_exponential_FDP_labeling_hybrid(
