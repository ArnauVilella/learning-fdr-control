...
def process_system_beta(args):
    """
    Process a single beta system.
    ...
    # Sample a and b for the beta distribution
    a = np.random.uniform(a_range[0], a_range[1])
    b = np.random.uniform(b_range[0], b_range[1])
    if a <= 0 or b <= 0:
        raise ValueError("a and b must be positive.")

    # Generate X from beta distribution, then standardize
    X_raw = np.random.beta(a, b, size=(n, p))
...
def generate_beta_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], a_range=(0.5, 5.0), b_range=(0.5, 5.0), show_progress=True, 
    ...
    """
    Generates data with beta predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        param_order = [
            ...
            f"p={p}", f"K={K}", f"a_min={a_range[0]}", f"a_max={a_range[1]}",
            f"b_min={b_range[0]}", f"b_max={b_range[1]}",
        ]
        folder_prefix = f"(beta_multilevel_{multilevel})_" if multilevel > 1 else "(beta)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_beta, system_args), total=len(local_systems), 
            ...
            results = [process_system_beta(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate beta data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--a_min', type=float, default=0.5, help='Minimum a for beta distribution')
    parser.add_argument('--a_max', type=float, default=5.0, help='Maximum a for beta distribution')
    parser.add_argument('--b_min', type=float, default=0.5, help='Minimum b for beta distribution')
    parser.add_argument('--b_max', type=float, default=5.0, help='Maximum b for beta distribution')
    ...
    if args.a_min <= 0 or args.b_min <=0:
        comm = MPI.COMM_WORLD
        if comm.Get_rank() == 0:
            parser.error("a_min and b_min must be greater than 0.")
    
    generate_beta_FDP_labeling_hybrid(
