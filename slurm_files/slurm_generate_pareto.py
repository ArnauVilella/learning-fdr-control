...
def process_system_pareto(args):
    """
    Process a single Pareto system.
    ...
    # Sample Pareto parameter a
    a = np.random.uniform(a_range[0], a_range[1])
    m = 1.0

    # Generate X from Pareto distribution and standardize
    X_raw = (np.random.pareto(a, size=(n, p)) + 1) * m
    
    mean = (a * m) / (a - 1)
    variance = (a * m**2) / ((a - 1)**2 * (a - 2))
...
def generate_pareto_FDP_labeling_hybrid(data_dir="data", specific_folder=None, T_stop_max=1, 
                                           N_systems=100, n=75, p=150, K=100, num_act_range=[1, 10], 
                                           num_dummies=150, num_dummies_factor=1, SNR_range=[0.5, 3.0], 
                                           a_range=(2.5, 10.0), show_progress=True, 
    ...
    """
    Generates data with Pareto predictors using hybrid MPI + multiprocessing parallelism.
    ...
    if specific_folder is None:
        ...
        folder_prefix = f"(pareto_multilevel_{multilevel})_" if multilevel > 1 else "(pareto)_"
        if test:
    ...
    if local_systems:
        ...
        if use_multiprocessing:
            with Pool(processes=min(n_cores_per_node, len(local_systems))) as pool:
                ...
                pbar = tqdm(map_func(process_system_pareto, system_args), total=len(local_systems), 
            ...
            results = [process_system_pareto(args) for args in pbar]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate Pareto data for FDP prediction using hybrid MPI+multiprocessing')
    ...
    parser.add_argument('--a_min', type=float, default=2.5, help='Minimum shape parameter a for Pareto distribution')
    parser.add_argument('--a_max', type=float, default=10.0, help='Maximum shape parameter a for Pareto distribution')
    ...
    if comm.Get_rank() == 0:
        if args.a_min <= 2.0:
            parser.error("a_min must be greater than 2.0 for finite variance.")
    
    generate_pareto_FDP_labeling_hybrid(
